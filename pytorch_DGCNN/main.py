...
def read(dir, name ,num):
    with open(dir+"/"+name+"_lines_data"+str(num)+".pkl", "rb") as f:
        train_lines = pickle.load(f)
    return train_lines
def loop_dataset_gem(classifier, type, dir, hub_num, args, optimizer=None):
    total_loss = []
    all_targets = []
    all_scores = []
    data = []
    ...
    for i in range(hub_num+1):
        data.extend(read(dir, type, i))
        if (i+1) % 10 == 0 or i == hub_num:
            print(type+str(i))
    ...
    AUC, AUPR, AUPR_norm = Evaluation(all_targets, all_scores)
    return avg_loss, AUC, AUPR, AUPR_norm
