...
def  subgraph_extraction_labeling(ind, A, h=1, max_nodes_per_hop=None, node_information=None):
    ...
    # g1 = nx.from_scipy_sparse_matrix(subgraph)
    # # remove link between target nodes
    # # if g1.has_edge(0, 1):
    # #     g1.del_edge(0, 1)
    # sub1 = nx.to_scipy_sparse_array(g1, format='csc')
    # g2 = nx.from_scipy_sparse_matrix(sub1)
    # apply node-labeling
...
def to_linegraphs(batch_graphs, max_n_label, tl, name):
    graphs = []
    pbar = tqdm(batch_graphs, unit='iteration')
    i = 0
    num = 0
    for graph in pbar:
        edges = graph.edge_pairs
        edge_feas = edge_fea(graph, max_n_label)/2
        node_feture = F.normalize(graph.node_features, p=2, dim=1)/2

        edges, feas = to_undirect(edges, edge_feas, node_feture)
        # edges, feas = to_undirect(edges, edge_feas, None)
        ...
        i += 1
        if i % 512 == 0:
            with open(tl + name + str(num) + '.pkl', "wb") as f:
...
def to_undirect(edges, edge_fea, node_features):
    edges = np.reshape(edges, (-1, 2))
    sr = np.array([edges[:, 0], edges[:, 1]], dtype=np.int64)
    fea_s1 = edge_fea[sr[0, :], :]
    fea_s = fea_s1.repeat(2, 1)
    fea_r1 = edge_fea[sr[1, :], :]
    fea_r = fea_r1.repeat(2, 1)

    fea_s2 = node_features[sr[0, :], :]
    fea_r2 = node_features[sr[1, :], :]
    #no_fea = fea_r2 + fea_s2
    no_fea = torch.cat([fea_r2, fea_s2], dim=1)
    no_fea = no_fea.repeat(2, 1)
    fea_body = torch.cat([fea_s, fea_r, no_fea], 1)
